...
interface Filters {
  [key: string]: string[]; // Ключ - имя фильтра, значение - массив значений
}
const ProductsByCategory = () => {
  ...
  const handleDisplayClick = (displayType: "lines" | "squares") => {
      ...
      try {
        ...
        const productsData: Product[] = productsSnapshot.docs.map((doc) => {
          ...
          return {
            ...
            companyName: data.companyName || "Неизвестная компания",
            price: data.price || 0,
            discount: data.discount,
            mainImage: data.mainImage,
            shortSpecs: data.shortSpecs || [],
  ...
  const handleSortOrder = (order: "asc" | "desc" | "default") => {
  ...
  const extractFilters = (products: Product[]) => {
    ...
    Object.entries(filters).forEach(([key, valuesSet]) => {
  ...
  const handlePriceInputChange = (type: "min" | "max", value: number) => {
    ...
    setSliderValue(newRange as [number, number]);
  };
  ...
  const availableCompanies = [
    ...new Set(products.map((product) => product.companyName).filter(Boolean)),
  ];
