...
const ProductsByCategory = () => {
    ...
    const fetchProducts = async () => {
      ...
      try {
        ...
        const prices = productsData.map((product) =>
          Number(product.discount || product.price)
        )
  ...
  const applyFiltersAndSorting = () => {
    ...
    tempProducts = tempProducts.filter((product) => {
      const priceToFilter = Number(product.discount || product.price)
      return priceToFilter >= minSliderValue && priceToFilter <= maxSliderValue
    })

    if (selectedCompanies.length > 0) {
      tempProducts = tempProducts.filter((product) =>
        selectedCompanies.includes(product.company_name)
      )
    }
    ...
    if (sortOrder === 'asc') {
      tempProducts.sort(
        (a, b) => Number(a.discount || a.price) - Number(b.discount || b.price)
      )
    } else if (sortOrder === 'desc') {
      tempProducts.sort(
        (a, b) => Number(b.discount || b.price) - Number(a.discount || a.price)
      )
    }
  ...
  const extractFilters = (products) => {
    ...
    products.forEach((product) => {
      if (product.full_specs) {
        product.full_specs.forEach(({ name, value }) => {
              ...
              <div className="filter-group">
                ...
                <div className="filter-values">
                  {products.length > 0 ? (
                    [
                      ...new Set(
                        products.map((product) => product.company_name)
                      ),
                    ].map((company, index) => (
                      <div
                        key={`${company}-${index}`}
                        ...
                        <div className="product-values">
                          ...
                          {product.short_specs &&
                            product.short_specs.length > 0 && (
                              <div>
                                <ul>
                                  {product.short_specs.map((spec, index) => (
                                    <li key={index}>
